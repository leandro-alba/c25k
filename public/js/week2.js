//const { deleteWeek2 } = require("../../controllers/week2")
...
Array.from(deleteBtn).forEach((el)=>{
    el.addEventListener('click', deleteWeek2)
})

async function deleteWeek2(){
    const week2Id = this.parentNode.dataset.id
    try{
        const response = await fetch('week2/deleteWeek2', {
            method: 'delete',
            headers: {'Content-type': "application/json"},
            body: JSON.stringify({
                'week2IdFromJSFile' : week2Id
            })
        })
